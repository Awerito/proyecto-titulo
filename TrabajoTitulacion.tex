...
susceptible al sesgo del observador del examen, es decir, es muy sensible a pérdida o
...
\subsubsection{Modelo Centernet}

% TODO: citar centernet
...
\chapter{Modelo de Identificación y Clasificación de Parásitos}
...
de parásitos presentes en la misma como se meustra en la figura \ref{fig:labelImage}. La
estructura de los datos se conforma en una carpeta raíz con las imágenes y su meta data en un
archivo en formato \textit{xml}, ambos de mismo nombre.
...
entrenamiento (\textit{train}) y testeo (\textit{test}), a su vez se requiere de una
modificación en la meta data de los archivos \textit{xml} que incluya si la carpeta del
archivo corresponde a uno de entrenamiento o testeo, agregar una etiqueta con el nombre del
archivo de la imagen que le corresponde y su ruta absoluta al mismo mostrado en la figura
\ref{fig:diff}. El conjunto de datos se separa con $60\%$ de las imágenes para entrenamiento del modelo y
$40\%$ para testeo de manera aleatoria uniforme.
...
la lista de parásitos faltantes los siguientes:
...
% TODO: Resultado final del dataset, como quedó la estructura de carpetas (mostrar), no hubo
% estratificación, estructura del código
\begin{figure}[ht]
    \centering
...
% TODO: datos de justificación de la decisión
...
El modelo es entrenado con el optimizador Adam y un ratio de aprendizaje de $0.001$ constante
durante todo el entrenamiento y $40000$ iteraciones por generación, especificando el modelo
en detección sin \textit{kfolds}, validación cruzada y sin optimización de hiperparámetros.
...
\chapter{Resultados}
% TODO: conectar el orden de los parrafos
En la gráfica de pérdida final de la figura \ref{fig:total_loss} se muestra el seguimiento de
...
% TODO: describir metricas en marco teorico
Las medidas de evaluación de precisión utilizadas son \textit{coco detection metrics}. Los
resultados del modelo concluyen en un tiempo de espera por inferencia de $7.84$ segundos con
clasificaciones mostradas a continuación en \ref{fig:comparation_caca}.
...
el examen seriado de deposiciones. Se vió el estado de los procedimientos para este examen
...
presencia parasitaria con la finalidad del entrenamiento del modelo. El conjunto de datos
cumplió con las características de poseer tres clases de parásitos intestinales, así como
también imágenes sin presencia alguna, haciendo una totalidad de 1217 imágenes de muestras.
...
%Se termina con un modelo de inteligencia artificial usando Centernet que tiene la capacidad
%de detectar y clasificar tres clases de parásitos diferentes en imágenes microscópicas de
%muestras de heces para el examen parasitológico seriado de deposiciones.

%Sus comparativas directas con modelos similares orientados a el uso de visión por computadora
%para análisis de imágenes microscópicas ...
